...
- 地理データを扱うAPIとDB設計
- AIのみでレビューまで完結させる開発、複数プロダクトを同時に修正するための仕組み作り
...
- AI開発の仕組み作り。JetBrains MPSでDSLを読み込ませてClaudeのコンテキストを90%削減し、Claude自身用のKanbanで忘却を防ぐなど、AgentやGuidelineを設計
...
- 決済システム、国内/海外配送のドメイン知識
- モブプロの導入(既存メンバーしか知らないドメイン知識を新規メンバーへ共有し、受け入れをしやすくした)
...
オンプレ・クラウドのサーバ構築、ネットワーク設計、フロントエンド、バックエンド、デザインまで扱える。社員時代にはプロジェクトリーダーとして、リリース計画の立案から進行管理、会議のファシリテーションまで担当した。

**分からないことを曖昧にしない**

仕様の不明点や疑問点は早い段階で質問し、率先して仕様や技術を把握する。結果として、どのプロジェクトでも早い段階で中心メンバーとして動けている。
